...
exclude_temporary_tables = 1  #  1 = yes, 0 = no
...
def get_correct_object(input):
    ind_obj_lst = input.split(".")
    actual_object = []
    for x in ind_obj_lst:
        if (x.find("]")==-1) :
            actual_object.append(x)
        elif (x.find("]")!=-1):
            exclude_alias = x[:x.find("]")]
            exclude_sq_brac = exclude_alias.replace("[","").replace("]","")
            actual_object.append(exclude_sq_brac)
    output = (".").join(actual_object)
    return output


empty_junk_before_next_run(junk_folder_path)
read_and_save_as_txt(source_folder_path)
tbl_to_view_known_list= {
"IFM_SF_DLY_CTRCT_DETAILS_CW" : "IFM.VW_IFM_SF_DLY_CTRCT_DETAILS_CW",
...
"IFM_SF_SPEC_CAP_EXP_OPEN_FUNDING_CW_HEAL" : "IFM.VW_IFM_SF_SPEC_CAP_EXP_OPEN_FUNDING_CW_HEAL"
...
for filename in os.listdir(junk_folder_path):
    ...
    for item in output_list:
        if(item not in correct_obj_list):
            correct_obj_list.append(get_correct_object(item))
    if(exclude_temporary_tables == 1):
        tmp_tbl_excluded_list = [item for item in correct_obj_list if not item.lstrip().startswith('#')] 
        filtered_list = [item for item in tmp_tbl_excluded_list if len(item) > 2]
    else:
        filtered_list = [item for item in correct_obj_list if len(item) > 2]
